...
export default function Students({ lang }) {
  ...
  const assignPackage = async (studentId) => {
    ...
    if (existing) {
      alert(lang === 'ar' ? 'هذه الباقة مُعيَّنة بالفعل' : 'This package is already assigned')
      setLoading(false)
      return
    }
                  ...
                  {packages.map(p => {
                    ...
                    return (
                      <option key={p.id} value={p.id} disabled={isAssigned}>
                        {p.name} ({p.lessons} {l.lessonsLeft}) {isAssigned ? '✓' : ''}
                      </option>
                    )
                  })}
                </select>
                <button onClick={() => assignPackage(selected.id)} disabled={loading || !pkgForm.packageId}
                  className="bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 text-white px-4 py-2 rounded-lg text-sm font-semibold transition whitespace-nowrap">
                  {loading ? '...' : l.assign}
                </button>
              </div>
            </div>
